/*
    ...
    <INSERT>
        ...
        1) INSERT INTO 테이블명 VALUES(값, 값, 값, ..., 값);
           테이블의 모든 컬럼에 대한 값을 INSERT하고자 할 때 사용한다.
           컬럼 순분을 지켜서 VALUES에 값을 나열해야 한다.
        2) INSERT INTO 테이블명(컬럼, 컬럼, ..., 컬럼) VALUES(값, 값, ..., 값);
           테이블에 내가 선택한 컬럼에만 값을 INSERT하고자 할 때 사용한다.
           선택이 안된 컬럼들은 기본적으로 NULL 값이 들어간다. (단, NOT NULL 제약조건이 걸려있는 컬럼은 반드시 선택해서 값을 INSERT해야 한다.)
           기본값(DEFAULT)이 지정되어 있으면 NULL이 가닌 기본값이 들어간다.
...
-- EMP_DEPT 테이블에는 EMPLOYEE 테이블의 부서 코드가 D1인 직원들의 사번, 직원명, 부서 코드, 입사일 삽입하고
...
-- EMPLOYEE 테이블에 노옹철 사원의 부서코드 D0으로 변경
UPDATE EMPLOYEE
SET DEPT_CODE = 'D0'
WHERE EMP_NAME = '노옹철';
